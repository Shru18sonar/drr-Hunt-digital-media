...
const Table = () => {
    ...
    const calMonthsBetween = (startDate, endDate) => {
    ...
    const calDaysBetween = (startDate, endDate, excludedDates) => {
        if (!startDate || !endDate) {
            return 0;
        }
        const start = new Date(startDate);
    ...
    const handleSaveButtonClick = (rowId) => {
        const updatedTableData = tableData.map((row) => {
            if (row.id === rowId) {
                const months = calMonthsBetween(rowInputData.startDate, rowInputData.endDate);
                const days = calDaysBetween(rowInputData.startDate, rowInputData.endDate, row.excludedDates);
